...
SELECT FIRST_NAME, DEPARTMENT_ID, JOB_ID FROM EMPLOYEES WHERE DEPARTMENT_ID IN 50;
...
SELECT FIRST_NAME, SALARY FROM EMPLOYEES WHERE DEPARTMENT_ID IN 60 ORDER BY SALARY DESC;
SELECT FIRST_NAME, JOB_ID FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' OR JOB_ID = 'SA_MAN';
SELECT FIRST_NAME || ' ' ||LAST_NAME || '사원의 급여는 ' || SALARY || '$ 입니다' AS SALARY FROM EMPLOYEES WHERE SALARY = 24000;
